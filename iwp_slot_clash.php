...
<section class="text-center py-5 bg1">
 <div class="container">
	<h1 style="font-weight: bold;font-family:verdana;">Slot Clash Re-Registration</h1>
 </div>
<hr>
<div class="container">
...
$sql1="select * from all_course1 where course_code='$course_code'";
...
echo '<table border="1" align="center">';
	echo '<tr>
		...
		for ($i=0;$i<$num_results1;$i++)
		{
			...
			$sql1_slot1=$row1['slot1'];
			$sql1_slot2=$row1['slot2'];
			$sql1_faculty_name=$row1['faculty_name'];
			$sql1_course_code=$row1['course_code'];
			$sql1_s_no=$row1['s_no'];
			$sql1_seat_avl=$row1['seat_avl'];
			$credit=$row1['credit'];


			$sql2="SELECT s_no, slot1, slot2, course_code from timetable_$reg_no";
			...
			for ($j=0;$j<$num_results2;$j++)
			{
				...
				$tslot1=$row2['slot1'];
				$tslot2=$row2['slot2'];
				$tcourse_code=$row2['course_code'];
				if ( $tslot1==$sql1_slot1 || $tslot1==$sql1_slot2 || $tslot2==$sql1_slot1 || $tslot2==$sql1_slot2 )
				{
			...
			{
				echo '<input type="radio" name="slot" value="'.$sql1_s_no.'">';
				echo '<label for="seat">'.$row1['seat_avl'].'</label>';
			}
			echo '</td>';
			echo '</tr>';
			a:
...
$num_results1 = mysqli_num_rows($result1);
		...
		for ($i=0;$i<$num_results1;$i++)
		{
			...
			for ($j=0;$j<$num_results2;$j++)
			{
				...
				$sum = $sum + $sql1_seat_avl;
			b:
		}
		if($sum==0)
		 {
				echo '<script>alert("No Seats Available....Your being redirected to Home page!")</script>';
